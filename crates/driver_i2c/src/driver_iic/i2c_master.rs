...
use crate::i2c::{
    FI2cConfig,FI2cFrameTX,FI2cFrameRX,FI2c
};
use crate::i2c_hw;

const FI2C_DATA_MASK (((!0u32) - (1u32 << 0) + 1) & (!0u32 >> (32 - 1 - 7)))
...
pub fn fi2c_master_start_trans(
    ...
    let mut addr_len:u32 = mem_byte_len;
    let mut ret = 0;
    ...
    while addr_len > 0 {
        if FI2cWaitStatus(base_addr, (0x1 << 1)) != 0 {
            break;
        }
        if input_32(base_addr,0x80) != 0 {
            return false;
        }
        if input_32(base_addr,0x70) & (0x1 << 1) != 0 {
            addr_len -= 1;
            let value = (mem_addr >> (addr_len * 8)) & FI2C_DATA_MASK;
            if addr_len != 0 {
                output_32(base_addr , 0x10, value);
            } else {
                output_32(base_addr , 0x10, value + flag as u32);
            }
        }
    }
    true
...
pub fn FI2cMasterStopTrans(instance_p: &FI2c) -> bool {
    assert!(instance_p.is_some());
    let mut ret = 0;
    let base_addr = instance_p.config.base_addr;
    let mut reg_val = 0;
    ...
    while true{
        if input_32(base_addr,0x34) & (0x1 << 9) {
            reg_val = input_32(base_addr,0x60);
            break;
        }
        else if 500 < timeout{
            break;
...
pub fn FI2cMasterReadPoll(
    ...
    while tx_len > 0 || rx_len > 0 {
        ...
        let rx_limit = 8 - input_32(base_addr , 0x78);
        let tx_limit = 8 - input_32(base_addr , 0x74);
        ...
        let mut rx_tem:u8 = input_32(base_addr,0x78)
        while rx_len > 0 && rx_tem > 0 {
            if input_32(base_addr,0x70) & (0x1 << 3) != 0 {
                *buf_p = (input_32(base_addr,0x10) & FI2C_DATA_MASK) as u8;
                buf_p = &mut buf_p[1..];
                rx_len -= 1;
                rx_tem -= 1;
                trans_timeout = 0;
        ...
        while tx_limit > 0 && buf_idx > 0 {
            if input_32(base_addr,0x70) & (0x1 << 1) != 0 {
                let reg_val = if buf_idx == 1 {
                    (FI2C_DATA_MASK & buf_p[0] as u32) | (0x0 << 8) | (0x1 << 9)
                } else {
                    (FI2C_DATA_MASK & buf_p[0] as u32) | (0x0 << 8)
                };
                output_32(base_addr,0x10, reg_val);
                buf_idx -= 1;
                tx_limit -= 1;
...
pub fn FI2cMasterReadIntr(
    ...
    output_32(instance_p.config.base_addr,0x38,0);
    ret = FI2cMasterStartTrans(instance_p, mem_addr, mem_byte_len, (0x0 << 8));
    instance_p.status = 0x2;
    if ret != 0{
        return ret;
    }
    let mut mask = input_32(instance_p.config.base_addr,0x30);
    mask |= (((0x1 << 4) | (0x1 << 6)) | (0x1 << 2));
    //need finish //FI2cMasterSetupIntr(instance_p, mask);
}
