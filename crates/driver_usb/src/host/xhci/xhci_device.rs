    ...
    O: OsDep,
{
        ...
        ) -> Result<ring::trb::event::TransferEvent>,
    {
        ...
        self.fetch_desc_endpoints().iter().for_each(|ep| {
         ...
         // 打印 Input64Byte 结构体内容
    debug!("Input64Byte structure after configuration:");
    debug!("Control:");
    debug!("  Add Context Flags: 0x{:x}", control_mut.add_context_flags());

    for (index, endpoint) in input.device().endpoints.iter().enumerate() {
        if let Some(endpoint) = endpoint {
            debug!("  Endpoint {}:", index + 1);
            debug!("    Interval: {}", endpoint.interval());
            debug!("    Endpoint Type: {:?}", endpoint.endpoint_type());
            debug!("    Max Packet Size: {}", endpoint.max_packet_size());
            debug!("    Max Burst Size: {}", endpoint.max_burst_size());
            // 打印其他相关字段
        }
    }
        ...
        let post_cmd = post_cmd(Allowed::ConfigureEndpoint({
            ...
            // }
            let input_context_pointer = (input as *mut Input64Byte).addr() as u64;
            debug!("Input Context Pointer: {:#x}", input_context_pointer);
            debug!("Slot ID: {}", self.slot_id);
            debug!("Configure Endpoint: {:?}", configure_endpoint);
            configure_endpoint
        }));
