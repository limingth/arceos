...
//         while self.current < self.current_len && self.raw[self.current] > 0 {
//             if let Some(desc_type) = self.peek_desc_type() {
//                 match desc_type {
//                     StandardDescriptorTypes::Device => {
...
//         if let Some(desc_type) = self.peek_desc_type() {
//             match desc_type {
//                 StandardDescriptorTypes::Interface => {
...
//                     Ok(TopologicalUSBDescriptorFunction::Interface(interfaces))
//                 }
//                 StandardDescriptorTypes::InterfaceAssociation => {
//                     trace!("parse InterfaceAssociation desc!");
//                     let interface_association = self.parse_interface_association().unwrap();
//                     let mut interfaces = Vec::new();
...
//         let mut vec = Vec::new();
//         while self.peek_desc_type() != Some(StandardDescriptorTypes::Endpoint) {
//             // match self.parse_any_descriptor().unwrap() {
//             //     ignore => {
...
//     fn cut_raw_descriptor(&mut self) -> Result<Vec<u8>, Error> {
//         if self.current < self.current_len && self.raw[self.current] > 0 {
...
//     fn peek_desc_type(&mut self) -> Option<StandardDescriptorTypes> {
//         let peeked = StandardDescriptorTypes::from_u8(self.raw[self.current + 1] as u8);
//         trace!("peeked type:{:?}", peeked);
//         peeked
//     }
...
//         if self.peek_desc_type() == Some(StandardDescriptorTypes::Interface) {
